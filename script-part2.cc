...
main (int argc, char *argv[])
{
	for(int n = 0; n <= 4; n++) {
	  bool verbose = true;
	  bool rayleigh = false; // change depending on if Rayleigh fading is tested or not
	  ...
	  mobility.SetPositionAllocator ("ns3::RandomDiscPositionAllocator", // allocates random position within a "disk"
									 "X", DoubleValue (10.0), 
									 "Y", DoubleValue (10.0),
									 "Rho", StringValue ("ns3::UniformRandomVariable[Min=0|Max=10]")); // the radius of the circle
