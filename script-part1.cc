...
main (int argc, char *argv[])
{
	for(int n = 0; n <= 4; n++) {
	  bool verbose = true;
	  bool rayleigh = true; // change to false to remove Rayleigh fading
	  ...
	  wifiStaNodes.Create (1); // number of station nodes
	  ...
	  mobility.SetPositionAllocator ("ns3::GridPositionAllocator", //places nodes in a grid with distance between nodes and grid width defined below, for 2 nodes can be used to easily place them at a set distance apart
									 ...
									 "DeltaX", DoubleValue (100.0), //nodes will be place 5m apart in x plane (changed this manually, since I could not make it work using command line arguments)
									 "DeltaY", DoubleValue (10.0),
									 "GridWidth", UintegerValue (3), 
									 "LayoutType", StringValue ("RowFirst"));
	  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel"); //nodes shouldn't move once placed (WayPointMobilityModel? - see API)
